...
namespace NTS_Reader_CS.xml
{
    class F103 : Dbconn //퇴직연금
    {
        ...
        public struct 상품별반복
        {
            ...
            public string pension_cd { get; set; } //계좌유형

            public int ann_tot_amt { get; set; } //당해연도납입금액
            public int tax_year_amt { get; set; } //당해연도인출금액
            public int ddct_bs_ass_amt { get; set; } //순납입금액계

            public int isa_ddct_bs_ass_amt { get; set; } //isa순납입급액
        ...
        public void Execute(F103 entity)
        {
            try
            { 
            if (entity.인별 == null)
                ...
                foreach (var 인별 in entity.인별)
            {
                ...
                foreach (var data in 인별.상품)
                {
                    ...
                    ISA_전체합계 += data.isa_ddct_bs_ass_amt;
            ...
            executeSql($@"                                      
                                UPDATE QE020MS
                                SET YCAL_TAXD_11_RETI_DAMT = {전체합계} -{ISA_전체합계},
                                    YCAL_TAXD_11_ISA_AMT= YCAL_TAXD_11_ISA_AMT+{ ISA_전체합계}
            ...
            {
                throw new Exception("퇴직연금[F103] 처리 중 오류가 발생하였습니다.");
    }
